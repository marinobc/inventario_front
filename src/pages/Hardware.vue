...
const filteredHardwareItems = computed(() => {
  ...
  return hardwareItems.value.filter((item) =>
...
async function fetchHardware() {
  ...
  try {
    console.log(
      "HardwarePage:fetchHardware:API call with filters",
...
async function handleSave() {
  ...
  try {
    if (isEditing.value) {
      await apiClient.put(
        `/hardware/${currentItem.value.id_hardware}`,
        currentItem.value
      );
    } else {
...
function showEditForm(item) {
  ...
  editableItem.fecha_mantenimiento = editableItem.fecha_mantenimiento
    ? new Date(editableItem.fecha_mantenimiento).toISOString().split("T")[0]
    : "";
  editableItem.fecha_baja = editableItem.fecha_baja
    ? new Date(editableItem.fecha_baja).toISOString().split("T")[0]
    : null;
  ...
  <PageWrapper title="Inventario de Hardware" :banner-image="bannerHardwareImage">
    ...
    <div class="filter-container">
      ...
      <div class="filter-buttons">
        <BaseButton class="btnClearF" @click="clearFilters" variant="secondary">
          Limpiar Filtros
        </BaseButton>

        <BaseButton class="btnClearF" @click="downloadCSV">
          Descargar CSV
        </BaseButton>
      </div>
    </div>
    ...
    <DataTable :items="filteredHardwareItems" :columns="tableColumns" :is-loading="isLoading" :actions-visible="authStore.permissions.hardware?.canUpdate ||
      ...
      <template #cell-fecha_baja="{ item }">
        {{
          item.fecha_baja
            ? new Date(item.fecha_baja).toLocaleDateString()
            : "N/A"
        }}
      </template>
